...
export default function IntroAnimation() {
  ...
  useEffect(() => {
    ...
    if (hasSeenIntro) {
      ...
      // Set a timeout to hide the intro after animation completes
  ...
  const handleAnimationComplete = () => {
    setTimeout(() => {
      setAnimationComplete(true)
    }, 500) // Short delay after exit animation
  }
                ...
                >
                  ...
                  >
                    <motion.div
                      ...
                      className="relative"
                    >
                      <Coffee className="absolute text-red-500 h-16 w-16" />
                      <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: [0, 1, 0] }}
                        transition={{ delay: 1.2, duration: 1.5, repeat: 1, repeatType: "reverse" }}
                      >
                        <Code className="text-blue-500 h-16 w-16" />
                      </motion.div>
                    </motion.div>
                  </motion.div>
                </motion.div>
